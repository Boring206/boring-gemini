...
# --- Quick Start (Vibe Coder Mode) ---
@app.command("quick-start")
def quick_start(
    idea: str = typer.Argument(..., help="你想要建立什麼？用自然語言描述"),
    template: str = typer.Option(
        None,
        "--template",
        "-t",
        help="使用專案模板 (fastapi-auth, nextjs-dashboard, cli-tool, vue-spa)",
    ),
    auto_approve: bool = typer.Option(False, "--yes", "-y", help="自動批准計畫，不需確認"),
):
    """
    🚀 一鍵啟動完整開發流程 (Vibe Coding 模式)
    ...
    """
    from .quickstart import QuickStartOrchestrator
    ...
    if result.get("phases", {}).get("execute", {}).get("status") == "error":
        raise typer.Exit(1)


# --- Templates CLI ---
templates_app = typer.Typer(help="管理專案模板 (Vibe Coder)")
...
def templates_list():
    """列出所有可用的專案模板。"""
    from .quickstart import list_templates

    templates = list_templates()

    console.print("[bold blue]📦 可用專案模板：[/bold blue]\n")
    for t in templates:
        console.print(f"  [cyan]{t['id']}[/cyan]")
        console.print(f"    {t['name']} - {t['description']}")
        console.print(f"    [dim]技術棧: {t['stack']}[/dim]\n")


@templates_app.command("show")
def templates_show(
    template_id: str = typer.Argument(..., help="模板 ID (如 fastapi-auth)"),
):
    """顯示模板詳細內容。"""
    from .quickstart import get_template

    template = get_template(template_id)

    if not template:
        console.print(f"[red]❌ 找不到模板: {template_id}[/red]")
        console.print("[dim]使用 'boring templates list' 查看可用模板。[/dim]")
        raise typer.Exit(1)
    ...
    console.print(
        Panel(
            ...
            f"[yellow]描述:[/yellow] {template['description']}\n"
            f"[yellow]技術棧:[/yellow] {template['stack']}\n\n"
            f"[yellow]Prompt 模板:[/yellow]\n{template['prompt_template']}",
            title=f"📦 {template_id}",
            border_style="cyan",
        )
    )
