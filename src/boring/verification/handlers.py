...
def verify_syntax_python(file_path: Path, project_root: Path, tools: ToolManager) -> VerificationResult:
    """Check Python syntax using compile()."""
    try:
        content = file_path.read_text(encoding="utf-8")
        compile(content, str(file_path), 'exec')
        return VerificationResult(
            passed=True, check_type="syntax", message=f"Syntax OK: {file_path.name}", details=[], suggestions=[]
        )
    except SyntaxError as e:
        return VerificationResult(
            passed=False, check_type="syntax", message=f"Syntax Error in {file_path.name}",
        ...
        return VerificationResult(passed=False, check_type="syntax", message=f"Error checking {file_path.name}: {e}", details=[str(e)], suggestions=[])

def verify_syntax_node(file_path: Path, project_root: Path, tools: ToolManager) -> VerificationResult:
    ...
    try:
        ...
        return VerificationResult(passed=False, check_type="syntax", message=f"Syntax Error: {file_path.name}", details=[result.stderr], suggestions=["Check JS syntax"])
    except Exception as e:
...
def verify_syntax_go(file_path: Path, project_root: Path, tools: ToolManager) -> VerificationResult:
    if not tools.is_available("go"): return VerificationResult(passed=True, check_type="syntax", message="Skipped", details=[], suggestions=[])
    try:
        result = subprocess.run(["go", "fmt", str(file_path)], stdin=subprocess.DEVNULL, capture_output=True, text=True, timeout=10)
        if result.returncode == 0: return VerificationResult(passed=True, check_type="syntax", message=f"Syntax OK: {file_path.name}", details=[], suggestions=[])
        return VerificationResult(passed=False, check_type="syntax", message=f"Go Syntax Error: {file_path.name}", details=[result.stderr], suggestions=["Run 'go fmt'"])
    except Exception as e: return VerificationResult(passed=False, check_type="syntax", message=f"Error: {e}", details=[], suggestions=[])

def verify_syntax_rust(file_path: Path, project_root: Path, tools: ToolManager) -> VerificationResult:
    if not tools.is_available("rustc"): return VerificationResult(passed=True, check_type="syntax", message="Skipped", details=[], suggestions=[])
    try:
        outfile = "/dev/null" if os.name != 'nt' else "NUL"
        result = subprocess.run(["rustc", "--emit=metadata", "-o", outfile, str(file_path)], stdin=subprocess.DEVNULL, capture_output=True, text=True, timeout=30)
        if result.returncode == 0: return VerificationResult(passed=True, check_type="syntax", message=f"Syntax OK: {file_path.name}", details=[], suggestions=[])
        return VerificationResult(passed=False, check_type="syntax", message=f"Rust Syntax Error: {file_path.name}", details=[result.stderr[:500]], suggestions=["Run cargo check"])
    except Exception as e: return VerificationResult(passed=True, check_type="syntax", message=f"Error: {e}", details=[], suggestions=[])

def verify_syntax_java(file_path: Path, project_root: Path, tools: ToolManager) -> VerificationResult:
    if not tools.is_available("javac"): return VerificationResult(passed=True, check_type="syntax", message="Skipped", details=[], suggestions=[])
    try:
        result = subprocess.run(["javac", "-Xlint:none", "-d", str(file_path.parent), str(file_path)], stdin=subprocess.DEVNULL, capture_output=True, text=True, timeout=30)
        if result.returncode == 0: return VerificationResult(passed=True, check_type="syntax", message=f"Syntax OK: {file_path.name}", details=[], suggestions=[])
        return VerificationResult(passed=False, check_type="syntax", message=f"Java Syntax Error: {file_path.name}", details=[result.stderr[:500]], suggestions=["Check Java syntax"])
    except Exception as e: return VerificationResult(passed=True, check_type="syntax", message=f"Error: {e}", details=[], suggestions=[])

def verify_syntax_c(file_path: Path, project_root: Path, tools: ToolManager) -> VerificationResult:
    if not tools.is_available("gcc"): return VerificationResult(passed=True, check_type="syntax", message="Skipped", details=[], suggestions=[])
    try:
        result = subprocess.run(["gcc", "-fsyntax-only", str(file_path)], stdin=subprocess.DEVNULL, capture_output=True, text=True, timeout=15)
        if result.returncode == 0: return VerificationResult(passed=True, check_type="syntax", message=f"Syntax OK: {file_path.name}", details=[], suggestions=[])
        return VerificationResult(passed=False, check_type="syntax", message=f"C Syntax Error: {file_path.name}", details=[result.stderr[:500]], suggestions=["Check C syntax"])
    except Exception as e: return VerificationResult(passed=True, check_type="syntax", message=f"Error: {e}", details=[], suggestions=[])

def verify_syntax_cpp(file_path: Path, project_root: Path, tools: ToolManager) -> VerificationResult:
    if not tools.is_available("g++"): return VerificationResult(passed=True, check_type="syntax", message="Skipped", details=[], suggestions=[])
    try:
        result = subprocess.run(["g++", "-fsyntax-only", "-std=c++17", str(file_path)], stdin=subprocess.DEVNULL, capture_output=True, text=True, timeout=15)
        if result.returncode == 0: return VerificationResult(passed=True, check_type="syntax", message=f"Syntax OK: {file_path.name}", details=[], suggestions=[])
        return VerificationResult(passed=False, check_type="syntax", message=f"C++ Syntax Error: {file_path.name}", details=[result.stderr[:500]], suggestions=["Check C++ syntax"])
    except Exception as e: return VerificationResult(passed=True, check_type="syntax", message=f"Error: {e}", details=[], suggestions=[])
...
def verify_lint_python(file_path: Path, project_root: Path, tools: ToolManager, auto_fix: bool = False) -> VerificationResult:
    ...
    if auto_fix:
        try:
            fix_result = subprocess.run(["ruff", "check", str(file_path), "--fix", "--unsafe-fixes"], stdin=subprocess.DEVNULL, capture_output=True, text=True, timeout=30, cwd=project_root)
    ...
    try:
        ...
        return VerificationResult(passed=True, check_type="lint", message=f"Error: {e}", details=[], suggestions=[])
...
def verify_lint_generic(file_path: Path, project_root: Path, tools: ToolManager, auto_fix: bool = False) -> VerificationResult:
    ...
    try:
        ...
        return VerificationResult(passed=False, check_type="lint", message=f"{tool_key} found issues", details=[result.stdout or result.stderr], suggestions=[f"Run {tool_key}"])
    except Exception as e:
        return VerificationResult(passed=True, check_type="lint", message=f"Error: {e}", details=[], suggestions=[])
...
def verify_imports_python(file_path: Path, project_root: Path) -> VerificationResult:
    try:
        ...
        return VerificationResult(passed=True, check_type="import", message=f"Skipped: {e}", details=[], suggestions=[])
...
def verify_imports_go(file_path: Path, project_root: Path, tools: ToolManager) -> VerificationResult:
