...
def run_tests_maven(project_root: Path, tools: ToolManager, test_path: Path = None) -> VerificationResult:
    ...
    try:
        ...
        return VerificationResult(passed=False, check_type="test", message="mvn test failed", details=[result.stdout[:500]], suggestions=["Fix Maven tests"])
    except Exception as e:
...
def run_tests_gradle(project_root: Path, tools: ToolManager, test_path: Path = None) -> VerificationResult:
    ...
    try:
        ...
        return VerificationResult(passed=False, check_type="test", message="gradle test failed", details=[result.stdout[:500]], suggestions=["Fix Gradle tests"])
    except Exception as e:
