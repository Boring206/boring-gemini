...
IMPLEMENTATION_PLAN_RUBRIC = Rubric(
    ...
    criteria=[
        Criterion("Completeness", "All requirements are addressed; no missing features or edge cases.", 1.5),
        Criterion("Feasibility", "The plan can be realistically implemented with available resources.", 1.2),
        Criterion("Clarity", "Steps are clear and unambiguous; no room for misinterpretation.", 1.0),
        Criterion("Risk Assessment", "Potential risks and mitigation strategies are identified.", 0.8),
        Criterion("Dependencies", "External dependencies and integration points are clearly defined.", 1.0),
...
TESTING_RUBRIC = Rubric(
    ...
    criteria=[
        ...
        Criterion("Isolation", "Tests are independent; no shared state that causes flakiness.", 1.2),
        Criterion("Assertions", "Assertions are specific and meaningful, not just 'no error'.", 1.0),
        Criterion("Maintainability", "Tests are readable and easy to update when code changes.", 0.8),
        Criterion("Performance", "Tests run quickly; no unnecessary delays or resource consumption.", 0.8)
    ],
    strictness="balanced"
)
...
DOCUMENTATION_RUBRIC = Rubric(
    ...
    criteria=[
        ...
        Criterion("Formatting", "Consistent formatting; proper use of headings, lists, code blocks.", 0.8)
    ]
)
...
PERFORMANCE_RUBRIC = Rubric(
    ...
    criteria=[
        Criterion("Algorithmic Efficiency", "Appropriate data structures and algorithms for the task.", 1.5),
        ...
        Criterion("Async/Concurrency", "I/O bound operations use async; CPU bound use threading/multiprocessing.", 1.0),
        Criterion("Database Queries", "Queries are optimized; no N+1 problems; proper indexing assumed.", 1.0)
    ],
    strictness="strict"
)
...
API_DESIGN_RUBRIC = Rubric(
    ...
    criteria=[
        Criterion("Consistency", "Naming conventions and patterns are uniform across the API.", 1.2),
        Criterion("Intuitiveness", "API is easy to understand without extensive documentation.", 1.0),
        Criterion("Versioning", "API versioning strategy is clear and supports backward compatibility.", 0.8),
        Criterion("Error Responses", "Error responses are informative with proper HTTP status codes.", 1.0),
        Criterion("Idempotency", "Safe methods are idempotent; unsafe methods handle duplicates gracefully.", 1.0)
    ]
)
