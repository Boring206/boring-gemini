...
## 🌐 知識群集 (Knowledge Swarm Sync)

從 V11.0 開始，全域大腦支援透過 Git 進行自動化同步，這被稱為 **「知識群集 (Knowledge Swarm)」**。

-   **工具**：`boring_brain_sync(remote_url=...)`
-   **協作力量**：透過將您的全域大腦指向團隊倉庫，每位開發者的 Agent 都可以從他人那裡獲取成功的模式。
-   **自動傳播**：針對困難架構或配置 Bug 的解決方案會在幾分鐘內在「群體智慧」中傳播。
