...
- **遷移引擎 (`boring migrate`)**: 確保專案狀態具備向前相容的平滑演進能力。
- **V14.1 規模與優化 (Scale & Optimization)**:
    - **增量對帳器**: 基於檢查點的誠信掃描，大幅提升驗證效能。
    - **帳本自動滾動**: 具備雜湊鏈連結的封存機制，支援無限增長的專案帳本。
    - **狀態快照壓縮**: 支援 Gzip 壓縮，針對巨型專案優化空間與 I/O。
    - **預測性預熱**: 多執行緒背景加載大腦上下文，降低開發節點間的切換延遲。
...
- **新指令**:
    ...
    - `boring doctor`: 執行全面的系統健康檢查 (支援 `--fix` 與 `--optimize`)。
