...
class TestPatchingTools:
    ...
    def test_boring_apply_patch_success(self, tmp_path):
        ...
        with (
            patch(
                "boring.mcp.tools.patching.get_project_root_or_error", return_value=(tmp_path, None)
            ),
            ...
            result = boring_apply_patch(
                file_path="test.txt", search_text="hello world", replace_text="HELLO WORLD"
            )
    ...
    def test_boring_apply_patch_not_found(self, tmp_path):
        with (
            ...
            result = boring_apply_patch(
                file_path="missing.txt", search_text="foo", replace_text="bar"
            )
    ...
    def test_boring_apply_patch_ambiguous(self, tmp_path):
        ...
        with (
            ...
            result = boring_apply_patch(
                file_path="test.txt", search_text="same", replace_text="diff"
            )
            assert result["status"] == "ERROR"
