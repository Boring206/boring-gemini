...
class TestPolishNode:
    ...
    @pytest.mark.asyncio
    async def test_process_first_attempt(self, polish, context):
    ...
    @pytest.mark.asyncio
    async def test_process_max_attempts(self, polish, context):
    ...
    @pytest.mark.asyncio
    async def test_process_increments_attempts(self, polish, context):
        """Test that process increments attempt counter."""
        assert context.stats.get("polish_attempts", 0) == 0

        with patch.dict("sys.modules", {"boring.mcp.tools.vibe": None}):
            with patch("shutil.which", return_value=None):
                await polish.process(context)

        assert context.stats["polish_attempts"] == 1

    @pytest.mark.asyncio
    async def test_process_with_ruff_available(self, polish, context):
    ...
    @pytest.mark.asyncio
    async def test_process_with_ruff_failure(self, polish, context):
