...
class TestPatchingState:
    ...
    def test_on_enter(self, patching_state, mock_context):
        """Test state entry."""
        with patch("boring.loop.states.patching.log_status") as mock_log, \
    ...
    def test_handle_write_file_success(self, patching_state, mock_context):
        ...
        with patch.object(patching_state, "_process_write_file") as mock_write, \
    ...
    def test_handle_search_replace_success(self, patching_state, mock_context):
        ...
        with patch.object(patching_state, "_process_search_replace") as mock_replace, \
    ...
    def test_handle_report_status(self, patching_state, mock_context):
        ...
        with patch.object(patching_state, "_get_files_to_modify", return_value=[]), \
             patch("boring.loop.states.patching.log_status"):
            result = patching_state.handle(mock_context)
    ...
    def test_handle_no_files_modified(self, patching_state, mock_context):
        ...
        mock_context.function_calls = [
            {
                "name": "write_file",
                "args": {"file_path": "test.py", "content": "code"}
            }
        ]
        mock_context.files_modified = []
        mock_context.files_created = []
        ...
        with patch.object(patching_state, "_process_write_file"), \
    ...
    def test_next_state_success(self, patching_state, mock_context):
        ...
        with patch("boring.loop.states.patching.VerifyingState", create=True) as mock_verifying, \
    ...
    def test_next_state_failure(self, patching_state, mock_context):
        ...
        with patch("boring.loop.states.patching.RecoveryState", create=True) as mock_recovery, \
             patch.object(patching_state, "_record_metrics"):
    ...
    def test_get_files_to_modify(self, patching_state, mock_context):
        ...
        write_calls = [
            {"name": "write_file", "args": {"file_path": "file1.py"}}
        ]
        replace_calls = [
            {"name": "search_replace", "args": {"file_path": "file2.py"}}
        ]
    ...
    def test_process_write_file(self, patching_state, mock_context):
        ...
        with patch("boring.loop.states.patching.validate_file_path", return_value=mock_validation), \
            ...
            assert len(mock_context.files_created) > 0 or call["args"]["file_path"] in str(mock_context.files_created)
