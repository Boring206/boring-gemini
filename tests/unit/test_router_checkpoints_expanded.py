...
def test_router_checkpoint_comprehensive():
    ...
    for query, expected_tool, expected_action, expected_name in scenarios:
        ...
        assert result.matched_tool == expected_tool, (
            f"Failed on query: '{query}'. Got '{result.matched_tool}' instead of '{expected_tool}' (Confidence: {result.confidence:.2f})"
        )
        assert result.suggested_params.get("action") == expected_action, (
            f"Wrong action for query: '{query}'. Got '{result.suggested_params.get('action')}'"
        )
        if expected_name:
            assert result.suggested_params.get("name") == expected_name, (
                f"Wrong name for query: '{query}'. Got '{result.suggested_params.get('name')}'"
            )
