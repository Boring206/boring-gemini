...
from boring.workflow_manager import WorkflowManager, WorkflowMetadata, WorkflowPackage


class TestWorkflowPackage:
    def test_json_serialization(self):
        metadata = WorkflowMetadata(
            name="test-flow", version="1.0.0", description="Test", created_at=123.0
        )
...
class TestWorkflowManager:
    ...
    def test_install_workflow_url(self, mock_urlopen, manager):
        ...
        mock_response.read.return_value = json.dumps(
            {"metadata": {"name": "remote-flow", "version": "1.0"}, "content": "# Remote"}
        ).encode("utf-8")
    ...
    def test_install_workflow_local_file(self, manager):
        with (
            ...
            mock_read.return_value = json.dumps(
                {"metadata": {"name": "local-flow"}, "content": "# Local"}
            )
    ...
    def test_publish_workflow_success(self, mock_urlopen, manager):
        # Mock export first
        with (
            patch.object(manager, "export_workflow") as mock_export,
            patch("pathlib.Path.exists", return_value=True),
        ):
            mock_file = MagicMock()
