...
from boring.loop import LoopContext
from boring.loop.base import StateResult
from boring.loop.states import PatchingState, RecoveryState, ThinkingState
...
class TestHappyPathScenario:
    ...
    def test_agent_creates_file_successfully(
        self, temp_project: Path, mock_gemini_client: MockGeminiClient
    ):
        """
    ...
    def test_state_transitions_work_correctly(
        self, temp_project: Path, mock_gemini_success_response
    ):
        ...
        context.function_calls = [create_write_file_call("src/test.py", "# test")]
...
class TestErrorRecoveryScenario:
    ...
    def test_no_function_calls_triggers_recovery(
        self, temp_project: Path, mock_gemini_no_function_calls
    ):
    ...
    def test_max_retries_causes_exit(self, temp_project: Path):
        ...
        context = LoopContext(
            project_root=temp_project, log_dir=temp_project / "logs", max_retries=2
        )
        context.storage = SQLiteStorage(temp_project / ".boring_memory")
    ...
    def test_recovery_generates_format_prompt_for_no_function_calls(self, temp_project: Path):
...
class TestStateMachineIntegration:
    ...
    def test_full_success_flow(self, temp_project: Path, mock_gemini_success_response):
        """Test complete thinking → patching → verifying → exit flow."""
        # Setup context
        context = LoopContext(
            project_root=temp_project, log_dir=temp_project / "logs", verification_level="BASIC"
        )
        context.storage = SQLiteStorage(temp_project / ".boring_memory")
        context.memory = MemoryManager(temp_project)
        ...
        assert len(context.function_calls) > 0, f"No function calls extracted. Result: {result}"
...
class TestFileSystemIntegration:
    ...
    def test_write_file_creates_parent_directories(self, temp_project: Path):
        ...
        context.function_calls = [
            create_write_file_call("src/deeply/nested/module.py", "# Nested module")
        ]
    ...
    def test_search_replace_modifies_existing_file(self, temp_project: Path):
        ...
        context.function_calls = [
            create_search_replace_call(
                "src/existing.py", 'print("old message")', 'print("new message")'
            )
        ]
